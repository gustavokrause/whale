...
const PLANNERS = ["consensus", "single", "duo"];
...
function validateConfigPatch(b: Record<string, unknown>) {
  ...
  for (const k of ["model_plan", "model_route", "model_nominate"]) {
    if (k in b) {
  ...
  if ("planner" in b) {
    if (!PLANNERS.includes(b.planner as string)) throw new Error(`planner must be ${PLANNERS.join("|")}`);
