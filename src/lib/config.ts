...
const envDefaults = {
  ...
  // The model Caio reasons with to ROUTE work — nominate the right persona panel
  // for a plan, and pick the right reviser for a refine. Routing wisdom (seeing a
  // dump spans Finance+Strategy+Product) is the bottleneck, so this defaults high.
  model_nominate: process.env.WHALE_MODEL_NOMINATE || "opus",
  ...
  // Dynamic peer-consensus planner (Caio nominates → personas propose+nominate →
  // fixpoint convergence → monotone peer revision). Default ON; set 0 to fall
  // back to the legacy Augusto+Maria duo. Rollback = WHALE_CONSENSUS=0 + restart.
  consensus: process.env.WHALE_CONSENSUS !== "0",
  // Planner mode (UI-toggleable): "consensus" (multi-agent bench — deep persona
  // view) | "single" (one opus planner — cheap/fast) | "duo" (legacy).
  planner: process.env.WHALE_PLANNER || "consensus",
};
...
export const config = {
  ...
  models: {
    ...
    get nominate() {
      return pick("model_nominate") as string;
    },
  },
  autonomy: {
    get bypass() {
    ...
    get consensus() {
      return pickBool("consensus");
    },
    get protected() {
      return protectedList();
    },
  },
  // Planner mode: "consensus" (multi-agent bench — deep persona view) | "single"
  // (one opus planner — cheap/fast, Caio's baseline) | "duo" (legacy). UI-toggleable
  // (DB override) so you switch deep↔cheap without a restart.
  get planner() {
    return (pick("planner") as string).toLowerCase();
  },
...
export function configSnapshot() {
  return {
    ...
    models: {
      ...
      nominate: config.models.nominate,
    },
    autonomy: {
