...
export type Job = { id: string; kind: string; key: string; startedAt: number };
...
export function startJob(kind: string, key: string, fn: () => Promise<{ ok: boolean; note?: string }>): Job {
  const id = `${kind}:${key}:${Date.now()}`;
  const job: Job = { id, kind, key, startedAt: Date.now() };
  running.set(id, job);
  broadcast();
  (async () => {
    let ok = false;
