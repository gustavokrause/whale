...
> The strategy brain on top of krill. You dump anything; baleia captures it,
...
read-only to baleia). Execution engine is `krill` (separate repo, unchanged).
...
```
ai-team/ (SOURCE OF TRUTH)            baleia + krill (WHERE WORK HAPPENS)
──────────────────────────           ───────────────────────────────────
WHO thinks & HOW they decide   ──▶    capture → distill → plan → triage → execute
- AGENTS.md (routing, risk            baleia READS personas; never writes them.
  rubric, handoff, economy)           Personas never know krill/baleia exist.
- ai/professionals/*/
  context.md + rules.md
```

- `ai-team` stays **personas-only**. baleia depends on it; it does not depend on baleia.
- baleia depends on `krill` only through krill's **HTTP API** (Section 6).
- Dependency direction is strictly: `ai-team ← baleia → krill`. No cycles.

## 4. The sync layer — `persona-loader` (core of the design)

A single module in baleia, pointed at the personas repo via config:

```
PERSONAS_DIR=/Users/gustavokrause/code/ai-team
```

It parses that folder into three artifacts baleia consumes at runtime:
...
- Editing a persona in `ai-team` changes baleia's behavior on the next run.
...
baleia writes to krill over **HTTP** (krill binds `0.0.0.0:3000`, LAN, no auth):
...
- `POST /api/tasks/[id]/transition` — promote `BACKLOG → TODO` when baleia
...
hijack krill's MCP server (that's krill↔its-own-Claude). baleia is just another
HTTP client.
...
- **One-way sync** — baleia can't corrupt the persona strategy.
...
- baleia proposes; **krill still stages** (plan → review → implement → AI review →
  publish). baleia never bypasses krill's own safety, only sets its review flags.

## 9. Eval — override rate

The single metric that governs autonomy: **how often the user overrides baleia's
triage/plan.**
...
- **Phase 0 — Sync foundation.** `persona-loader`: read `ai-team/` → routing +
  risk rubric + persona registry. Prove baleia can assume any persona and apply
...
- **baleia runtime**: **separate process** beside krill (own port), talks to
...
- **CONTEXT.md ownership**: lives **inside baleia**, keyed by `project_id`
  (global living file + per-project). baleia owns its memory; user project repos
  stay clean.
...
- **Rafael (Backend)** — krill write surface, baleia ↔ krill integration, schema of `CONTEXT.md`.
