...
test("triage classifies risk correctly", () => {
  ...
  assert.equal(t("fix typo in readme", "client-app").risk_tier, "low");
  assert.equal(t("add a db migration", "client-app").risk_tier, "high", "irreversible keyword");
  assert.equal(t("change the pricing tier", "client-app").risk_tier, "high", "safe-word");
...
test("autonomy ladder: dial controls how far a task bypasses (B1)", () => {
  ...
  assert.equal(bypass(low, "client-app", "conservative"), false);
  assert.equal(bypass(med, "client-app", "conservative"), false);
  assert.equal(bypass(low, "client-app", "balanced"), true);
  assert.equal(bypass(med, "client-app", "balanced"), false);
  assert.equal(bypass(low, "client-app", "aggressive"), true);
  assert.equal(bypass(med, "client-app", "aggressive"), true);
});
...
test("B3 refine: Input re-evaluates + re-triages + logs the turn", async () => {
  resetDb();
  const t = addProposed({ project_key: "client-app", name: "add export", description: "csv", risk_tier: "medium" });
...
test("B4 arm-time confirm: auto-finish push/batch needs a distinct confirm", async () => {
  resetDb();
  const t = addProposed({ project_key: "client-app", name: "x", risk_tier: "low", auto_publish: true });
  ...
  const b = await pushBatch(stubTeam as never, "client-app");
...
test("auto-finish rung (A2): auto_publish only for aggressive + low + non-self-edit", () => {
  ...
  assert.equal(ap("fix typo", "client-app", "aggressive"), true, "aggressive low -> auto-finish");
  assert.equal(ap("fix typo", "client-app", "balanced"), false, "balanced low -> no auto-finish");
  assert.equal(ap("build a feature", "client-app", "aggressive"), false, "medium never auto-finishes");
...
test("autonomous rung: auto_publish for low+medium, NOT high, NOT self-edit", () => {
  ...
  assert.equal(tri("fix typo", "client-app").auto_publish, true, "low -> auto");
  assert.equal(tri("build a feature", "client-app").auto_publish, true, "medium -> auto");
  assert.equal(tri("add a db migration", "client-app").auto_publish, false, "high -> NOT auto");
  assert.equal(tri("add a db migration", "client-app").bypass, false, "high -> full review (no bypass)");
...
test("ludicrous rung: auto_publish for EVERY tier except self-edit", () => {
  ...
  assert.equal(ap("fix typo", "client-app"), true, "low -> auto");
  assert.equal(ap("build a feature", "client-app"), true, "medium -> auto");
  assert.equal(ap("add a db migration", "client-app"), true, "high (irreversible) -> auto");
  assert.equal(ap("change the pricing tier", "client-app"), true, "high (safe-word) -> auto");
...
test("self-edit guard: orchestrator tasks never bypass, any dial", () => {
  assert.equal(
    triage(stubTeam, { name: "fix typo", description: "", project_key: "client-app" }, "aggressive").bypass,
    true,
  );
...
test("blocker queue: file (deduped), list open, resolve", () => {
  ...
  addBlocker({ kind: "mcp_auth", trigger_kind: "plan", trigger_ref: "client-app", summary: "other" });
...
function mockKrill() {
  ...
  globalThis.fetch = (async (url: unknown, opts: { method?: string; body?: string } = {}) => {
    ...
    if (u.includes("/api/health")) data = {};
      ...
      { id: "proj1", slug: "ZT", name: "client-app", folder_path: "/x", has_repo: true },
...
test("WH-11 push(id): refuses when a dependency isn't in krill; passes depends_on once it is", async () => {
  ...
  try {
    ...
    const b = addProposed({ project_key: "client-app", name: "B", deps: ["A"] });
    ...
    const a = addProposed({ project_key: "client-app", name: "A" });
...
test("acceptance: stored on the proposed task and carried into the krill payload", async () => {
  ...
  try {
    ...
    const t = addProposed({ project_key: "client-app", name: "checkout persists plan", acceptance: acc });
...
test("acceptance: null when the planner didn't author one", async () => {
  ...
  const k = mockKrill();
  try {
    const t = addProposed({ project_key: "client-app", name: "no acceptance" });
...
test("WH-11 pushItems: in-batch dep pushes in order; a missing upstream defers, not strips", async () => {
  ...
  try {
    ...
    addProposed({ project_key: "client-app", name: "A" });
    addProposed({ project_key: "client-app", name: "B", deps: ["A"] });
    const r = await pushBatch(stubTeam as never, "client-app");
    ...
    resetDb();
    addProposed({ project_key: "client-app", name: "C", deps: ["ghost"] });
    const r2 = await pushBatch(stubTeam as never, "client-app");
...
test("push: all krill toggles carry into the payload (non-protected project)", async () => {
  ...
  const k = mockKrill();
  try {
    const t = addProposed({ project_key: "client-app", name: "toggles", bypass: true, auto_publish: true });
...
test("push: skip_verify omitted from payload when null (krill defaults by mode)", async () => {
  ...
  const k = mockKrill();
  try {
    const t = addProposed({ project_key: "client-app", name: "verify-auto" });
...
test("push: skip_verify=false (force on) is sent explicitly so krill verifies even a non-dev task", async () => {
  ...
  const k = mockKrill();
  try {
    const t = addProposed({ project_key: "client-app", name: "force-verify", mode: "non-dev" });
